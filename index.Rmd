...
-   **neuroticism**: Una medida de inestabilidad emocional, los valores están en la escale de Eysenck.
-   **extraversion**: Una medida del interes por el mundo exterior de la gente y de las cosas, los valores están en la escale de Eysenck.
...
Con ese valor, ya sabemos que el odds de ser mujer es `r round(abs(1-OR_MujHom),5)` veces el odds del hombre. El odds ratio (OR) puede ir de 0 a infinito. Un OR de 1 implica que no hay diferencias.
...
Veamos la tabla de contingencia de manera gráfica en la Figura \@ref(fig:mosaic).
...
El resultado anterior ha usado a *mujer* como *referencia*: nos informa cuánto afecta al log odds de ser voluntario el ser hombre en comparación con ser mujer (la referencia). La Tabla \@ref(tab:rlog1) nos confirma que ser hombre disminuye la probabilidad de ser voluntario. La Tabla \@ref(tab:rlog1a) recalcula la regresión con la categoría *hombre* como referencia.
...
Comencemos por calcular los coeficientes estandarizados [@ibm_spss_computing_2020] para saber cuál de los predictores del modelo III tiene mayor efecto (ver Tabla \@ref(tab:stdCoefs)).
...
Cuando usamos una regresión logística queremos poder predecir probabilidades, en esta caso, ante una combinación de los predictores X, qué **probabilidad** se obtiene para ser voluntario. Ese valor es el \*\*fitted\* del modelo, y nos servirá para evaluar si nos hemos acercado al Y original. Calculemos tales probabilidades, y cone ella creemos la Matriz de Confusión de la Tabla \@ref(tab:confusionMx).
...
confusionMatrix(vol$volunteer, predicted)%>%
    kable(caption = "Matriz de Confusión")%>%
     kableExtra::kable_styling(full_width = F)
```

La antidiagonal debería tener sólo 0s si tuviésemos una predicción perfecta. Si un modelo es mejor, debe reducir sustantivamente ese error. De la Matriz de Confusión podemos calcular algunos indicadores clave:

-   La **Sensitividad**: Predecir bien el caso verdadero (aquí predecir bien al 'ser voluntario').
...
-   La **Especificidad** : Predecir bien el caso falso (aquí predecir bien al 'no ser voluntario')
...
```{r, echo=TRUE}
ToInput3 <- data.frame(sex=factor(c("female","male")), 
...
Pero NO sabemos cuando afecta cada variable la probabilidad de ser voluntario en promedio. Para ello pedimos efectos marginales, los que podemos ver en la Tabla \@ref(tab:marginals-table)
...
marginalsData%>% kable(caption = "Efectos Marginales - Modelo III") %>%kableExtra::kable_styling(full_width = F)

```

La Figura \@ref(fig:marginals-plot) podemos ver graficado la Tabla \@ref(tab:marginals-table).

```{r marginals-plot, fig.cap="Efectos Marginales"}
library(ggplot2)
